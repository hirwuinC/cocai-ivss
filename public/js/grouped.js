...
function agregaring(ingrediente,iding,receta){
		...
		.done(function(data) {
			$('#tmcant').empty();
			$('#tmcant').append('Agregar producto');
			$('#unidadmed').empty();
			$('#bot').empty();
	 ...
	 function agregado(ingrediente,iding,receta){
		...
		if (data == 'duplicado') {
				$('#cuerpoagregar').empty();
				$('#cuerpoagregar').append('<div class="alert alert-danger" style="text-align: left"><i style="float: right; color: black; font-size: 14px" class="fa fa-close" id="equis"></i></a><b>El producto que intenta agregar ya pertenece a esta receta, intente con un producto diferente</b></div>');
 ...
 function eliminaring(producto, iding, idreceta,ingrediente){
	 	...
	 	.done(function(data) {
		 	...
		 	$('#cuerpodelete').empty();
		 	$('#cuerpodelete').append('<p>Usted está por eliminar el producto '+data[0]['nombre']+' '+data[0]['marca']+' de este agrupado. ¿Desea continuar?</p>');
